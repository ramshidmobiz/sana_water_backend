...
class Product(models.Model):
    ...
    branch_id = models.ForeignKey('master.BranchMaster', on_delete=models.SET_NULL, null=True, blank=True,related_name='prod_branch')
    category_id = models.ForeignKey('master.CategoryMaster', on_delete=models.SET_NULL, null=True, blank=True,related_name='prod_category')
    fiveg_status = models.BooleanField(default=False)
    tax = models.ForeignKey('tax_settings.Tax', on_delete=models.SET_NULL, null=True, blank=True,related_name='prod_branch')
...
class Product_Default_Price_Level(models.Model):
    ...
    product_id = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True, blank=True,related_name='customer_product_rate')
    rate = models.CharField(max_length=50)
    class Meta:
...
class Staff_Orders_details(models.Model):
    ...
    product_id = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True, blank=True,related_name='staff_product')
    C_STATUS = (
        ('Order Request', 'Order Request'),
        ('Cancelled', 'Cancelled'),
        ('Pending', 'Pending'),
...
class Staff_IssueOrders(models.Model):
    ...
    order_number = models.CharField(max_length=30, null=True, blank=True, unique=True)
    salesman_id = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, blank=True, related_name='salesman_orders')
    ...
    product_id = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='issued_products')
